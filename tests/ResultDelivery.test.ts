...
describe('bounded result-delivery ledger and private retry capability', () => {
  ...
  test('a pending result retries against the one server behind the game', async () => {
    const { store, record } = await completedStore();
    const storage = new MemoryStorage();
    const capabilities = new ResultDeliveryCapabilityStore(storage, capabilityClock);
    capabilities.remember(record.id, capability, record.completedAt!);
    const primaryPost = vi.fn(async () => accepted({ duplicate: true }));
    const service = new ResultDeliveryService(store, capabilities, () => fakeClient(primaryPost));
  ...
  test('stored secondary-endpoint fields are ignored rather than creating a fallback', async () => {
    const { record } = await completedStore();
    const capabilities = new ResultDeliveryCapabilityStore(new MemoryStorage(), capabilityClock);
    expect(
      capabilities.remember(
    ...
    expect(capabilities.get(record.id)).toEqual(capability);
  });
});
