...
describe('the bonus', () => {
  ...
  test('changing the converting team moves the bonus with it', () => {
    ...
    fireEvent.click(screen.getByText('20'));
    openEditor();
    expect(screen.getByText(/Bonus — NINETY SIX/)).toBeTruthy();
   ...
   *
   * `Edit parts…` used to turn each part into one number box for the controlling team and, under
  ...
  test('bonus parts ask who took each one, with the teams named on screen', () => {
    ...
    expect(within(parts).getByText('Ninety Six')).toBeTruthy();
    expect(within(parts).getByText('Greenwood bounceback')).toBeTruthy();
  ...
  /**
   * Opening the parts view is a way of looking at the bonus, not a way of clearing it.
   *
   * It used to fill the parts with zeros, so pressing Edit parts… on a recorded 20 rewrote the bonus
   * to 0 — and now that a part shows a chosen outcome rather than an empty box, that also asserted
   * something false on screen: three Misses on a bonus that scored 20.
   */
  test('opening the parts view keeps the bonus that was recorded', () => {
    ...
    // Two parts to Ninety Six and the third bounced: the only decomposition a total leaves open.
    expect(screen.getByLabelText('Bonus part 1 to Ninety Six').getAttribute('aria-pressed')).toBe('true');
    expect(screen.getByLabelText('Bonus part 2 to Ninety Six').getAttribute('aria-pressed')).toBe('true');
    expect(screen.getByLabelText('Bonus part 3 bounced back to Greenwood').getAttribute('aria-pressed')).toBe('true');
    expect(within(score).getByRole('row', { name: /Ninety Six: unchanged at \+30/ })).toBeTruthy();
