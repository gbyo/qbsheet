...
describe('Director integration hardening', () => {
  ...
  test('administrative manual settlement retires the live native QBTCP session first', async () => {
    ...
    // The settlement below is a canonical result, which only lands in a released round.
    releaseRoundForResults(hook, scheduled.roundId);
    await waitForDurable(hook);
  ...
  test('start and low-level release both recheck packet reuse', async () => {
    ...
    let released = true;
    await act(async () => {
      released = await Promise.resolve(hook.result.current.releaseRound(secondRound.id));
    });
    expect(released).toBe(false);
    expect(hook.result.current.error).toMatch(/Shared|Packet|Round 1/);
