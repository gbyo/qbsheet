...
function GeneralSettings({
  ...
  const details = tournamentDraft.key === tournamentKey
    ? tournamentDraft
    : {
        key: tournamentKey,
        name: state.tournament?.name ?? '',
  ...
  const operator = operatorDraft.key === operatorKey
    ? operatorDraft
    : {
        key: operatorKey,
        name: operatorProfile?.displayName ?? 'Local operator',
  ...
  const saveTournament = () => {
    ...
    onAnnounce(saved ? 'Tournament details updated locally; saving now.' : errorNotice('Tournament details were not updated; review the Director error.'));
  };
    ...
    <div className="director-stack">
      ...
      >
        {state.tournament ? (
          ...
          >
            <FieldGrid>
              <Field label="Name"><TextInput value={details.name} onChange={(event) => updateDetails({ name: event.target.value })} /></Field>
              <Field label="Venue" optional><TextInput value={details.venue} onChange={(event) => updateDetails({ venue: event.target.value })} /></Field>
              <Field label="Date" render={({ id, describedBy }) => <DateField id={id} aria-describedby={describedBy} value={details.date} onChange={(event) => updateDetails({ date: event.target.value })} />} />
              <Field label="End date" optional hint="For multi-day tournaments." render={({ id, describedBy }) => <DateField id={id} aria-describedby={describedBy} value={details.endDate} onChange={(event) => updateDetails({ endDate: event.target.value })} />} />
              <Field label="Organizer" optional><TextInput value={details.organizer} onChange={(event) => updateDetails({ organizer: event.target.value })} /></Field>
              <Field label="Question set" optional><TextInput value={details.questionSet} onChange={(event) => updateDetails({ questionSet: event.target.value })} placeholder="e.g. ACF Fall 2025" /></Field>
              <Field
            ...
            <div className="director-form-actions"><Button variant="primary" type="submit">Save tournament details</Button></div>
          </form>
        ) : (
          <EmptyState title="No tournament open" description="Open or create a tournament before editing tournament details." />
        )}
      </Panel>
      ...
      >
        <FieldGrid>
          <Field label="Display name"><TextInput value={operator.name} onChange={(event) => setOperatorDraft({ ...operator, key: operatorKey, name: event.target.value })} /></Field>
          <Field label="Role" optional><TextInput value={operator.role} onChange={(event) => setOperatorDraft({ ...operator, key: operatorKey, role: event.target.value })} placeholder="Tournament director" /></Field>
        </FieldGrid>
        <div className="director-form-actions">
          <Button
            ...
            onClick={() => {
              onSaveOperator?.({ displayName: operator.name.trim(), role: operator.role.trim() || undefined });
  ...
  return (
        ...
        <Button
          ...
          onClick={() => {
            void controller
              ...
              .catch((reason: unknown) => onAnnounce(errorNotice(reason instanceof Error ? reason.message : 'Recovery point could not be saved.')));
          }}
        >
    ...
    >
      {controller.repositoryKind === 'memory' && (
        <Callout tone="warning">This Director session is using memory-only storage. Recovery points disappear when the session ends.</Callout>
      )}
      {checkpoints.length === 0 ? (
        <div className="director-empty-in-panel"><p className="director-empty-copy">No recovery points yet.</p></div>
      ) : (
                ...
                <Button
                  ...
                  onClick={() => void (async () => {
                    const approved = await confirmAction({
                      ...
                      consequence: 'The open tournament will be replaced by the selected recovery point. Operator settings and credentials stay unchanged.',
                      confirmLabel: 'Restore tournament',
                      tone: 'danger',
                    });
                    if (!approved) return;
                    const restored = await controller.restoreCheckpoint(entry.id);
                    onAnnounce(restored ? 'Tournament restored. The previous state is also available in Recovery.' : errorNotice('The tournament could not be restored; review the Director error.'));
                  })()}
                >
                  Restore…
                </Button>
...
function AuditHistory({ state }: { state: DirectorState }) {
  ...
  if (state.audit.length === 0) {
    return <EmptyState title="No audit history yet" description="Meaningful tournament and Director decisions will be retained here." />;
  }
  return (
    <Panel
      id="settings-audit"
    ...
    >
      ...
      {visibleAudit.length < state.audit.length && (
        <div className="director-panel-footer">
          <Button variant="secondary" onClick={() => setAuditShown((shown) => shown + auditPageSize)}>Load more</Button>
          <span className="director-text-meta">{state.audit.length - visibleAudit.length} earlier event{state.audit.length - visibleAudit.length === 1 ? '' : 's'}</span>
        </div>
      )}
    </Panel>
  );
...
function SystemDiagnostics({
  ...
  const storageLabel = controller.repositoryKind === 'tauri-sqlite'
  ...
  return (
    <div className="director-stack">
      <Panel
        ...
        actions={<StateLabel state={controller.error ? 'help' : controller.saving ? 'pending' : 'finished'} label={controller.error ? 'Needs attention' : controller.saving ? 'Saving' : 'Healthy'} />}
      >
        <p className="director-text-secondary">
          {state.metadata.lastSavedAt ? `Last saved ${new Date(state.metadata.lastSavedAt).toLocaleString()}.` : 'This tournament has not been saved yet.'}
        </p>
      ...
      <Diagnostics
        ...
        items={[
          ...
          { term: 'Last saved', value: state.metadata.lastSavedAt ? new Date(state.metadata.lastSavedAt).toLocaleString() : 'Never' },
          { term: 'Audit events', value: state.audit.length },
        ]}
      />
      <AdvancedSection label="Local network boundaries" hint="QBTCP room control and QBSheet Live use separate listeners and credentials." icon="network">
        <p>Start and inspect QBTCP from Rooms & staff and QBSheet Live from its own destination. Public spectator URLs never contain management credentials.</p>
      </AdvancedSection>
    </div>
  );
}

function settingsSectionForTarget(target: DirectorNavigationTarget | null | undefined): SettingsSection | null {
