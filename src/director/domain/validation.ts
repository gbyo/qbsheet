...
export function runPreflight(
  ...
  for (const round of state.rounds) {
    if (!roundScheduleIsValid(state, round.id)) {
      // A later-phase round may exist as plan structure before advancement
      // supplies its field. That empty round is deferred work, not a
      // tournament-wide readiness failure; the current phase stays strict.
      if (deferredEmptyRoundIsExpected(state, round.id)) continue;
      issues.push({
