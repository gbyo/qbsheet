...
export function advancedFromBasic(basic: IBasicScoringRulesInput): IAdvancedScoringRulesInput {
  ...
  if (basic.powerValue !== undefined) {
    answerTypes.push(newAdvancedAnswerType({ value: basic.powerValue, label: 'Power', shortLabel: 'P' }));
  }
  answerTypes.push(newAdvancedAnswerType({ value: basic.tossupValue, label: 'Correct', shortLabel: 'C' }));
...
export function advancedFitsBasicForm(input: IAdvancedScoringRulesInput): boolean {
  ...
  return input.answerTypes.every((type) => type.awardsBonus === (type.value ?? 0) > 0);
}
